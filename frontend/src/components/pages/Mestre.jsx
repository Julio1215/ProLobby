...
// - Esta página já estava bem escrita — sem React Query, sem Zustand.
// - O arquivo foi apenas movido de components/pages/MestrePage.jsx para pages/Mestre.jsx.
// - Nenhuma lógica foi alterada.
//
// Como funciona:
// O usuário escolhe um tipo (NPC, Cenário, Monstro, Quest, Encontro, Chat Livre),
// um sistema de RPG e escreve sua ideia.
// Ao clicar em "Gerar", chamamos a API da Anthropic diretamente com fetch().
// O resultado aparece como um card de histórico da sessão.
// O histórico é apenas estado local (useState) — some quando a página é recarregada.
...
const SISTEMAS = ['D&D 5e', 'Tormenta 20', 'Call of Cthulhu', 'Ordem Paranormal', 'Pathfinder 2e']

// Monta o prompt adequado para cada tipo de conteúdo
const PROMPTS = {
...
// Chama a API da Anthropic com o prompt montado
...
// Renderiza markdown simples sem biblioteca externa
...
// Card que exibe um resultado gerado
function CardResultado({ item }) {
  const [copiado, setCopiado] = useState(false)
  const tabInfo = TABS.find(t => t.key === item.tab)

  function copiar() {
    navigator.clipboard.writeText(item.resultado)
    setCopiado(true)
    setTimeout(() => setCopiado(false), 2000)
  }

  return (
    <div style={{ background: 'var(--color-dark-card)', border: '1px solid var(--color-dark-border)', borderRadius: '20px', overflow: 'hidden' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.875rem 1.25rem', borderBottom: '1px solid var(--color-dark-border)', background: 'rgba(124,58,237,0.06)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
          <span>{tabInfo?.icon}</span>
          <span style={{ fontSize: '0.82rem', fontWeight: 700, color: 'var(--color-brand-light)' }}>{tabInfo?.label}</span>
          <span style={{ fontSize: '0.7rem', color: 'rgba(240,238,255,0.35)', borderLeft: '1px solid rgba(124,58,237,0.2)', paddingLeft: '0.6rem' }}>
            {item.sistema} · {item.horario}
          </span>
        </div>
        <button onClick={copiar}
          style={{ padding: '0.3rem 0.75rem', borderRadius: '8px', fontSize: '0.72rem', fontWeight: 600, background: copiado ? 'rgba(16,185,129,0.15)' : 'rgba(124,58,237,0.12)', border: `1px solid ${copiado ? 'rgba(16,185,129,0.3)' : 'rgba(124,58,237,0.25)'}`, color: copiado ? '#10b981' : 'var(--color-brand-light)', cursor: 'pointer', transition: 'all 0.2s' }}>
          {copiado ? '✓ Copiado' : '📋 Copiar'}
        </button>
      </div>

      <div style={{ padding: '0.75rem 1.25rem', borderBottom: '1px solid rgba(124,58,237,0.08)', background: 'rgba(0,0,0,0.15)' }}>
        <p style={{ fontSize: '0.72rem', color: 'rgba(240,238,255,0.35)', fontStyle: 'italic' }}>"{item.input}"</p>
      </div>

      <div style={{ padding: '1.25rem' }}>
        <Markdown text={item.resultado} />
      </div>
    </div>
  )
}
...
export default function Mestre() {
  ...
  async function handleGerar() {
    ...
    try {
      const prompt = PROMPTS[tab]({ sistema, input: input.trim() })
      ...
      setErro(err.message || 'Erro ao conectar com a IA')
    } finally {
      setCarregando(false)
    }
  }
  ...
  function handleKey(e) {
    if (e.key === 'Enter' && (e.ctrlKey || e.metaKey)) handleGerar()
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div>
        ...
        <p style={{ fontSize: '0.82rem', color: 'var(--color-dark-text-2)' }}>
          Gere NPCs, cenários, monstros e quests com inteligência artificial
        </p>
      </div>

      {/* Painel de controle */}
      <div style={{ background: 'var(--color-dark-card)', border: '1px solid var(--color-dark-border)', borderRadius: '20px', padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>

        {/* Tabs de tipo */}
        <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
          {TABS.map(t => (
            <button key={t.key} onClick={() => setTab(t.key)}
              style={{ padding: '0.45rem 0.875rem', borderRadius: '99px', fontSize: '0.78rem', fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s', border: `1px solid ${tab === t.key ? 'var(--color-brand)' : 'var(--color-dark-border)'}`, background: tab === t.key ? 'rgba(124,58,237,0.15)' : 'transparent', color: tab === t.key ? 'var(--color-brand-light)' : 'var(--color-dark-text-2)' }}>
              {t.icon} {t.label}
            </button>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          {SISTEMAS.map(s => (
            <button key={s} onClick={() => setSistema(s)}
              style={{ padding: '0.3rem 0.7rem', borderRadius: '8px', fontSize: '0.72rem', fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s', border: `1px solid ${sistema === s ? 'rgba(124,58,237,0.5)' : 'rgba(124,58,237,0.12)'}`, background: sistema === s ? 'rgba(124,58,237,0.12)' : 'transparent', color: sistema === s ? 'var(--color-brand-light)' : 'rgba(240,238,255,0.4)' }}>
              {s}
            </button>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: '0.68rem', color: 'rgba(240,238,255,0.3)' }}>Ctrl+Enter para gerar</span>
            <button onClick={handleGerar} disabled={carregando || !input.trim()} className="btn btn-primary" style={{ fontSize: '0.85rem', minWidth: '140px' }}>
              {carregando ? (
      ...
      {/* Histórico de resultados gerados na sessão */}
      {historico.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <h2 style={{ fontSize: '1rem', fontWeight: 700 }}>📜 Gerados nesta sessão</h2>
          ...
          {historico.map(item => <CardResultado key={item.id} item={item} />)}
        </div>
      )}

      {/* Estado vazio inicial */}
      {!carregando && historico.length === 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '3rem 2rem', gap: '1rem', background: 'rgba(124,58,237,0.12)', border: '1px dashed rgba(124,58,237,0.2)', borderRadius: '20px' }}>
          <span style={{ fontSize: '3rem' }}>🎲</span>
          <div>
            <p style={{ fontWeight: 700, marginBottom: '0.4rem' }}>Pronto para criar aventuras épicas</p>
